...
sp.run(
    "docker build -f node.Dockerfile -t bxia68/macrostrat:node .".split()
)

sp.run(f"docker push bxia68/macrostrat:node".split())
...
for service in service_list:
    sp.run(llm_command.format(worker_node=service["host"], gpu_id=service["gpu_id"]).split())
    sp.run(worker_command.format(worker_node=service["host"], gpu_id=service["gpu_id"]).split())

time.sleep(5)

worker_list = [f"\"worker_{service['host']}_{service['gpu_id']}\"" for service in service_list]
sp.run(master_command.format(data_dir=DATA_DIR, worker_list=",".join(worker_list)).split())
